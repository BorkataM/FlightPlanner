...
import { isUpcoming, fmtTime, fmtDate, tripDurationMin, fmtDuration, type Trip } from './tripUtils'
...
export function LeftPanel({ nextTrip, upcoming, past, selected, onSelect }: {
...
// Right panel
export function RightPanel({ trip }: { trip: Trip }) {
  const navigate = useNavigate()
  const { t } = useLocale()
  const mb = t.myBookings
