...
const Tab = ({ icon, label, active, onClick }: { icon: React.ReactNode; label: string; active: boolean; onClick: () => void }) => (
  <button onClick={onClick}
    className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-medium transition-all ${active ? 'bg-white text-gray-900 shadow' : 'text-gray-400 hover:text-white'}`}>
    {icon}{label}
  </button>
)

const FilterChip = ({ label }: { label: string }) => (
  <button className="text-gray-400 hover:text-white text-xs font-medium transition-colors whitespace-nowrap">{label}</button>
)
...
export default function SearchBox() {
  ...
  return (
    <div className="search-glass rounded-2xl w-full max-w-[900px] shadow-2xl">
      {/* Tab row */}
      <div className="flex items-center justify-between px-4 pt-3 pb-3 border-b border-white/[0.06]">
      ...
      {/* Input row */}
      <div className="relative flex items-stretch border-b border-white/[0.06]">
        <div className={`flex-1 grid divide-x divide-white/[0.06] ${isRoundTrip ? 'grid-cols-[1fr_auto_1fr_1fr_1fr]' : 'grid-cols-[1fr_auto_1fr_1fr]'}`}>
        ...
        {browseField && (
          <div
            ...
            style={{ background: 'rgba(10,22,46,0.98)', border: '1px solid rgba(255,255,255,0.12)', backdropFilter: 'blur(24px)', borderTop: 'none' }}
          >
            {browseLoading ? (
              <div className="flex items-center gap-2 px-6 py-5 text-gray-500 text-sm">
              ...
              <div className="p-5 max-h-72 overflow-y-auto columns-3 gap-6">
                {airportsByCountry.map(([country, airports]) => (
                  <div key={country} className="break-inside-avoid mb-4">
                    <div className="text-[10px] text-gray-500 uppercase tracking-widest font-semibold mb-1.5">{country}</div>
                    {airports.map(a => (
                      <button
                        key={a.icaoCode}
                        onClick={() => handleBrowseSelect(a)}
                        className="flex items-center justify-between w-full py-1 px-1 rounded hover:bg-white/5 transition-colors text-left group"
                      >
                        <span className="flex items-center gap-1.5 text-sm text-white group-hover:text-gray-100">
                          <MapPin className="w-3 h-3 text-gray-600 shrink-0" />
                          {a.city}
                        </span>
                        <span className="text-gray-500 text-xs ml-2 shrink-0">{a.iataCode ?? a.icaoCode}</span>
                      </button>
                    ))}
                  </div>
                ))}
      ...
      <div className="flex items-center justify-between px-4 py-2.5">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-gray-400 font-medium">{t.flexibleDates}</span>
